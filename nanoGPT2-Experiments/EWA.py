...
for models in tqdm(selected_files, desc="Outer Loop"):
    ...
    for j, model_path in enumerate(models[1:]):
        ...
        # Extracting the number from the string using the refined pattern
        # number_pattern = re.compile(r'ckpt_(\d+).pt')
        # string = str(model_path)
        # ckpt_number = int(number_pattern.search(string).group())  # Current checkpoint number being processed
        # # # Extract the number from the string using the pattern
        # number = number_pattern.search(string).group()
        # Extract checkpoint number using the provided logic
        assert os.path.exists(model_path)
        number_pattern = re.compile(r'ckpt_(\d+).pt')
        string = str(model_path)
        ...
        # print('++++++++++++++++++++')
        # print(f'ckpt_number: {ckpt_number}')
        # print(f'last_ckpt_number: {last_ckpt_number}')
