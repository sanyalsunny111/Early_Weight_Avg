...
# data
dataset = 'openwebtext'
gradient_accumulation_steps = 5  # used to simulate larger batch sizes, was 5 earlier
batch_size = 12  # if gradient_accumulation_steps > 1, this is the micro-batch size, was 12 earlier
block_size = 1024
# model
n_layer = 12
n_head = 12
...
# train_indices = torch.randperm(len(train_data) - block_size)
...
if init_from == 'scratch':
    ...
    ckpt_path = os.path.join(out_dir, 'ckpt_52500.pt')
...
model_ema = SWA(
    ...
    update_every=swa_step,  # how often to actually update, to save on compute (updates every 10th .update() call)
)
...
def estimate_loss_ema():
    ...
    for split in ['train', 'val']:
        out[split] = {}
        losses = torch.zeros(eval_iters)
        for k in tqdm(range(eval_iters), desc="Evaluating Model", ncols=100):
            X, Y = get_batch(split)
            with ctx:
                logits, loss = model(X, Y)
            losses[k] = loss.item()
    ...
    for split in ['train', 'val']:
        losses_ema = torch.zeros(eval_iters)
        for k in tqdm(range(eval_iters), desc="Evaluating EMA Model", ncols=100):
            X, Y = get_batch(split)
            with ctx:
                logits_ema, loss_ema = model_ema(X, Y)
            losses_ema[k] = loss_ema.item()
...
swa_scheduler = SWALR(optimizer, anneal_epochs=(max_iters - iter_num), swa_lr=min_lr, anneal_strategy="cos")
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        log_text = f"step {iter_num}: train loss {losses['train']['vanilla']:.4f}, val loss {losses['val']['vanilla']:.4f}, train loss swa {losses['train']['ema']:.4f}, val loss swa {losses['val']['ema']:.4f}"

        with open("logs/swa-log/training_val_early.txt", "a") as log_file:
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        with open("logs/swa-log/train_log_early.txt", "a") as log_file:
        ...
        #         log_file.write(log_message + "\n")
        # else:
        #     # If the file already exists, just append the log message
